  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-[var(--border)] bg-[color-mix(in_oklab,var(--bg-elevated)_88%,transparent)] px-3 backdrop-blur-xl sm:px-4">
      ...
      <button
        ...
        className="group flex h-9 min-w-0 flex-1 items-center gap-2.5 rounded-md border border-[var(--border-strong)] bg-[var(--panel-muted)] px-3 text-left transition-colors hover:border-[var(--brass)] md:max-w-md"
      >
      ...
      <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
        ...
        <div className="hidden text-right lg:block">
          <div className="text-[12px] font-medium leading-tight">{formatDatePL(today, "long")}</div>
        ...
        >
          <button
            onClick={() => runSync()}
            disabled={syncing}
            className="flex h-9 items-center gap-2 rounded-md border border-[var(--border-strong)] bg-[var(--panel-muted)] px-2.5 text-[12px] transition-colors hover:border-[var(--brass)] disabled:opacity-70"
          >
          ...
          <DropdownMenuContent align="end" className="w-72">
              ...
              <DropdownMenuItem asChild>
                <Link href="/admin/booksy">
                  ...
                  <span className="flex-1">
                    {conflicts} konflikt{conflicts > 1 ? "y" : ""} synchronizacji
                  </span>
                </Link>
              </DropdownMenuItem>
            ) : null}
            <DropdownMenuItem asChild>
              <Link href="/admin/raport-kasowy">
                <Settings />
                <span className="flex-1">Raport kasowy za dziś nie zamknięty</span>
              </Link>
            </DropdownMenuItem>
            <DropdownMenuItem asChild>
              <Link href="/admin/ewidencja">
                <UserCog />
                <span className="flex-1">Ewidencja czasu — 4 wpisy do akceptacji</span>
              </Link>
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 rounded-md py-1 pl-1 pr-1.5 transition-colors hover:bg-[var(--panel-muted)]">
              <Avatar name="BroZone" ring="var(--brass)" className="size-7" />
              <div className="hidden text-left sm:block">
                <div className="text-[12px] font-medium leading-tight">BROZONE</div>
                <div className="text-[10px] text-[var(--fg-subtle)]">Owner</div>
              </div>
              <ChevronDown className="size-3.5 text-[var(--fg-subtle)]" />
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel>Konto właściciela</DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem asChild>
